...
const Form = () => {
  ...
  const login = async (values, onSubmitProps) => {
    const logUser = await fetch("http://localhost:3001/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values),
    });

    const logUserResponse = await logUser.json();
    onSubmitProps.resetForm();
    if (logUserResponse) {
      dispatch(
        setLogin({
          user: logUserResponse.searchUser,
          token: logUserResponse.token,
        })
      );
    }
    navigate("/home");
